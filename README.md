...
当前代码是 **M0 阶段**：纯静态前端，零依赖、零构建、无需 API Key，克隆下来即可运行。

```bash
git clone <repo> && cd ai-travel-decision-system
python3 -m http.server 8080
```

访问 `http://localhost:8080/` 生成计划，点击"打开出行 H5"查看分享页（H5 链接可直接分享，计划编码在 URL 中，无需服务端）。

**联网与否的差异**：

| 环境 | 表现 |
|---|---|
| 能访问 Open-Meteo | 天气实时拉取，正常输出适配度分数与风险信号 |
| 断网 / 接口不通 | 行程、航班示例、落地交通、行李清单、美食娱乐全部照常生成；适配度显示"日期超出可靠预报范围"而不是编造分数 |
